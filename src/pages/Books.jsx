...
import { Search, Plus, BookOpen, Edit3, Trash2, Filter, MoreVertical, Calendar, Hash, User, Copy } from "lucide-react"
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-xl p-6 border border-border/50 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{new Set(books.map((book) => book.author)).size}</p>
              </div>
              ...
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <h3 className="font-semibold text-lg text-foreground mb-1 line-clamp-2">{book.title}</h3>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-border">
              <h2 className="text-xl font-semibold text-foreground">{isEditing ? "Edit Book" : "Add New Book"}</h2>
              <p className="text-sm text-muted-foreground mt-1">
                {isEditing ? "Update book information" : "Enter the details for the new book"}
              </p>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-2">Copies Available</label>
                <input
                  type="number"
                  min="0"
                  value={formData.copies_available}
                  onChange={(e) => setFormData({ ...formData, copies_available: Number.parseInt(e.target.value) || 0 })}
                  className="w-full px-3 py-2 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors"
